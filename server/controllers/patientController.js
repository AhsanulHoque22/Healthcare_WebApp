...
const getPatientMedicalSummary = async (req, res, next) => {
  try {
    const patientId = req.params.id;
    const shouldReanalyze = ['true', '1', 'yes'].includes(String(req.query.reanalyze || '').toLowerCase());
    
    const summary = await clinicalService.getUnifiedMedicalSummary(patientId, shouldReanalyze);
    ...
    console.error('[patientController] getPatientMedicalSummary error:', error.message);
    next(error);
  }
};
